...
class k_space:
    '''k_space class, generates k-paths/grids for the calculations.
       ...
       Instance attributes:
       k_type        # Defines the called function for generating the k-space
                     # {path,plane,self-defined,monkhorst,sphere,sphere_ster_proj}
    ...
    def __init__(self,K_TYPE,K_BASIS,VECS,BRA_VEC=None,
        ...
        if self.k_type == "sphere_ster_proj":
            self.k_space_car,self.k_space_proj = self.sphere_ster_proj(
                self.vecs_car)
            self.k_space_red = self.car_to_red(self.k_space_car)
            self.k_kind = "mesh"

        if self.k_type == "sphere_ster_proj_plot":
            self.k_space_red, self.k_space_car = self.sphere_ster_proj_plot(
                self.vecs_car)
    ...
    def red_to_car(self,red):
        ...
        car = np.einsum("ij,...j->...i",
                        np.linalg.inv(self.bra_vec),
                        red)*2*np.pi
        return car

    def set_k_dist(self):
        '''Calculates the k-distance along a given connected path.
           P.E: Could be improved by doing this segment-wise
           => Non-connected paths...
        '''
        ...
        for i_k in range(np.shape(self.k_space_car)[0]):
            k_dist[i_k] = (k_dist[i_k-1]
                           + np.linalg.norm((self.k_space_car[i_k]-k_prev)))
    ...
    def Path(self,vecs):
        ...
        for vec in range(len(self.vecs)-1):
            for i in range(3):
                path[vec*self.n_points:(vec+1)*self.n_points,i] = np.linspace(
                    vecs[vec,i],vecs[vec+1,i],self.n_points)
        return path
    ...
    def Plane(self,vecs):
        '''Creates a plane CONTAINING THE MARGINS OF THE PLANE.
           ...
           The first two vectors define a path, which is shifted
           by the third one, to generate the grid.
        '''
    ...
    def Monkhorst(self,vecs):
        '''Creates a gamma centered Monkhorst grid.
           The input vector specifies the number of
           sampling points in each direction.
           '''
        ...
        def Gamma_centered(VEC):
            return (np.linspace(0,1,VEC,endpoint=False)+0.5)%1.0-0.5
        out[:,:,:,0] = Gamma_centered(vecs[0,0])[:,None,None] 
        out[:,:,:,1] = Gamma_centered(vecs[0,1])[None,:,None]
        out[:,:,:,2] = Gamma_centered(vecs[0,2])[None,None,:]   
    ...
    def sphere_ster_proj(self,k):
        ...
        if np.shape(k) == (1,3):
            ...
            print("North pole of the sphere points towards k_z-direction.")
              ...
              and np.linalg.norm(k[1]) == 1.0
              and np.linalg.norm(k[2]) == 1.0):
            v3 = k[1]
            v1 = k[2]
            print("North pole oriented along "+str(v3))
            print("Azimuthal vector starts in direction "+str(v1))
        else:
            print(("Input vector has the wrong shape",
                   "Please specify the center of the sphere",
                   "or give additionally north pole and",
                   "starting point of azimuthal vector"))
        ...
        def ster_proj(theta,phi):
            ...
            if theta == 0:
                theta = 0.000001  # small epsilon, map north-pole not to ifty
        ...
        v1 = np.array([1,0,0])
        v2 = np.cross(v3,v1)
        # sample on 0 and 2pi avoid north-pole which is projected to R->infty
        for i in range(self.n_points):
            for j in range(2*self.n_points):
                phi = 2*np.pi/(2*self.n_points-1)*j
                theta = 1*np.pi/(self.n_points-1)*(i)
                mesh[2*i*self.n_points+j] = (k[0] + self.radius*(
                    np.cos(theta)*v3 + np.sin(theta)*(
                        np.cos(phi)*v1 + np.sin(phi)*v2)))
                ste[i,j] = ster_proj(theta,phi)
        return mesh,ste

    def sphere_ster_proj_plot(self,vec):
        ''' For plotting of the ster-proj. This function defines a grid,
            with constant point density in the stereographic projection plane.
...
if __name__ == "__main__":
    ...
    print("Checking the transform from reduced to cartesian coordinates...")
    points = 5
    ...
    print("Checking the back transform from cartesian",
          "coodinates to reduced coordinates...")
    ...
    if np.array_equal(car_latt.vecs_red,vecs):
        print("Transformation between reduced and",
              "cartesian coordinates works correctly!!!")
    else:
        print("Transformation between reduced and,"
              "cartesian coordinates FAILES!!!")
    ...
    print("Checking function create_kPlane on " + str(points)
          + "x" + str(points) + " grid:")
    ...
    spheresterproj = k_space("sphere_ster_proj","car",
                             center,real_vec,points,radius)
    print(spheresterproj.__dict__.keys())
    print('Testing function "sphere_ster_proj_plot" ...')
    spheresterprojplot = k_space("sphere_ster_proj_plot","car",center,
                                 real_vec,points,radius)
